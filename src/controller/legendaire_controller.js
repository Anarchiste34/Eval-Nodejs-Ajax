const pathToData = "./src/model/pokémon.json"
const fs = require('fs');

 // POST 
exports.createLegendaire = (request, response) =>{
    // Lecture du fichier
    fs.readFile(pathToData, (err, data) =>{
        ...
        if(existingData.legendaire.length === 0) {
            //tableau = requete id = 1
             existingData.legendaire.push({ "id": 1, "name": request.body.name, "prix": request.body.prix });
            //sinon            
        } else {
            let thisData = existingData.legendaire[ existingData.legendaire.length-1 ];
            //tableau = requete id = taille du tableau + 1 
            existingData.legendaire.push({ "id": thisData.id + 1, "name": request.body.name, "prix": request.body.prix });
        }
...
exports.getLegendaire = (request, response) =>{
    ...
    // Sinon renvoi status 200 et le rendre en format json
                ...
                const existingData = JSON.parse(data).legendaire
            response.status(200).json(existingData)
        }
    });
};

exports.getLegendaireById = (request, response) =>{
    ...
    fs.readFile(pathToData, (err, data) =>{
        ...
        if(err){
            ...
            const dataId = manipData.legendaire.find(
...
exports.updateLegendaire = (request, response) =>{
    //Lecture du fichier
    fs.readFile(pathToData, (err,data) =>{
        ...
        const dataId = existingData.legendaire.find(
        (obj) => obj.id === parseInt(request.params.id)    
        );
...
exports.deleteLegendaireById = (request, response) => {
    //Lecture du fichier
    fs.readFile(pathToData, (err, data) =>{
    ...
    if (err) {
        ...
        if (!dataId) {
            ...
            existingData.legendaire = existingData.legendaire.filter(
                (obj) => obj.id != parseInt(request.params.id));

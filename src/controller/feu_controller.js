...
exports.getFeu = (request, response) =>{
    ...
    // Sinon renvoi status 200 et le rendre en format json
                ...
                const existingData = JSON.parse(data).feu
            response.status(200).json(existingData)
        }
    });
};

exports.getFeuById = (request, response) =>{
    ...
    fs.readFile(pathToData, (err, data) =>{
        ...
        if(err){
            ...
            const dataId = manipData.feu.find(
...
exports.updateFeu = (request, response) =>{
    //Lecture du fichier
    fs.readFile(pathToData, (err,data) =>{
        ...
        const dataId = existingData.feu.find(
        (obj) => obj.id === parseInt(request.params.id)    
        );
...
// POST 
exports.createFeu = (request, response) =>{
    // Lecture du fichier
    fs.readFile(pathToData, (err, data) =>{
        ...
        const existingData = JSON.parse(data);
        if(existingData.feu.length === 0) {
            //tableau = requete id = 1
             existingData.feu.push({ "id": 1, "name": request.body.name, "prix": request.body.prix });
            //sinon            
        } else {
            let thisData = existingData.feu[ existingData.feu.length-1 ];
            //tableau = requete id = taille du tableau + 1 
            existingData.feu.push({ "id": thisData.id + 1, "name": request.body.name, "prix": request.body.prix });
        }
...
exports.deleteFeuById = (request, response) => {
    //Lecture du fichier
    fs.readFile(pathToData, (err, data) =>{
    ...
    if (err) {
        ...
        if (!dataId) {
            ...
            existingData.feu = existingData.feu.filter(
                (obj) => obj.id != parseInt(request.params.id));
